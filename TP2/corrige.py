...
# Script de correction pour TP2 INF2171 
    ...
    def create_testinfos():
        ...
        for path in paths:
            for f in sorted(list(Path(path).glob('*.s'))):
                ...
                expected = m[1].encode('raw_unicode_escape').decode('unicode_escape')
                stdin=""
...
class Correction:
    ...
    def __init__(self, path, filename, testinfos):
...
def correction_un_fichier(path, filename, testinfos, path_correction=None):
    correction = Correction(path, filename, testinfos)
...
def main():
    ...
    parser.add_argument('paths', nargs='*', default=["vie.s"], help="Noms des fichiers à corriger, le fichier dans le dossier courant est utilisé par défault")
    ...
    if args.correcteur:
        ...
        correction_un_fichier(filename, filename, testinfos).show_all()
